...
(defclass extract-functions (cl-webgpu/shader/internal::shader-walker)
  ())
...
(defwalker extract-functions (defun name lambda-list &body body+d)
  (multiple-value-bind (body declare doc)
    ...
    (let* ((cl-webgpu/shader/internal::*current-function*
             (process-type-declarations-for-scope
              (add-function name lambda-list
                            nil
                            :declarations declare :docs doc)))
           (*function-stages* (valid-stages cl-webgpu/shader/internal::*current-function*)))
      (clrhash (bindings-used-by cl-webgpu/shader/internal::*current-function*))
      (when (boundp '*new-function-definitions*)
        (pushnew cl-webgpu/shader/internal::*current-function* *new-function-definitions*))
      (setf (body cl-webgpu/shader/internal::*current-function*)
            (with-lambda-list-vars (cl-webgpu/shader/internal::*current-function*) (@@ body)))
      ;; if *function-stages* is NIL, we got bindings that only exist
      ;; in disjoint sets of stages...
      (assert *function-stages*)
      (setf (valid-stages cl-webgpu/shader/internal::*current-function*)
...
(macrolet ((track-globals (&rest forms)
  ...
  (track-globals defconstant defparameter
                 cl-webgpu/shader/internal:defconstant cl-webgpu/shader/internal::%defconstant
                 cl-webgpu/shader/internal:attribute cl-webgpu/shader/internal:uniform
                 cl-webgpu/shader/internal:input cl-webgpu/shader/internal:output
                 cl-webgpu/shader/internal:bind-interface))
                           ...
                           (prog1
                             ...
                             (when (boundp '*new-type-definitions*)
                               (let ((sname (if (consp name) (car name) name)))
                                 (pushnew (list sname (get-type-binding sname))
...
;; struct-construction (from (make SomeStruct) in shader code):
;; visit the constructed struct type so it appears in dependency order.
(defmethod walk ((form struct-construction) (walker tree-shaker))
  (funcall *tree-shaker-type-hook* (construction-type form))
  (when (next-method-p)
    (call-next-method)))
...
(defclass update-calls (cl-webgpu/shader/internal::shader-walker)
                  ...
                  (defun main ()
                    ...
                    (let ((e)
                      ...
                      (if e
                          ...
                          (complicated (if f (cl-webgpu/shader/internal::<< f 1) (cl-webgpu/shader/internal::>> e 1)) (cl-webgpu/shader/internal::<< 4 +hoge-piyo+)
                                       :d 4)))))
                'main
                :vertex))
...
#++
(cl-webgpu/shader/internal::generate-stage :fragment 'skybox-shaders::fragment)
#++
(print (cl-webgpu/shader/internal::generate-stage :fragment 'cl-webgpu/shader-mesh-shaders::fragment))
#++
(print (cl-webgpu/shader/internal::generate-stage :geometry 'cl-webgpu/shader-mesh-shaders::tsd-geometry))
