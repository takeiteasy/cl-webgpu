...
;;     ex: *, textureSize
(defun add-internal-function/s (name lambda-list arg-types return-type &key target-name (cast t))
...
(defun add-internal-function/full (name lambda-list type &key target-name (cast t))
  (let* ((fn (make-instance 'internal-function
                            :name name
                            :target-name target-name
                            :lambda-list lambda-list
...
(labels ((make-ftype (ret &rest args)
  ...
  (let* ((*environment* cl-webgpu/shader/internal::*shader-base-environment*)
         ...
         (scalar (list :bool :int :uint :float))
         (number (list :int :uint :float))
         ...
         (log* (make-ftype (append gen-itype gen-utype
                           ...
                           (append gen-itype gen-utype
                                   ixv ivec uxv uvec)))
         (sampler '(:sampler-2d :sampler-3d :sampler-cube
                    :sampler-2d-shadow
                    :sampler-cube-shadow
                    :sampler-cube-array-shadow
                    :sampler-2d-array
                    :sampler-2d-array-shadow))
         (isampler '(:isampler-2d :isampler-3d :isampler-cube
                     nil
                     nil
                     nil
                     :isampler-2d-array
                     nil))
         (usampler '(:usampler-2d :usampler-3d :usampler-cube
                     nil
                     nil
                     nil
                     :usampler-2d-array
    ...
    #++
    (add-internal-function/s 'cl-webgpu/shader/internal::incf '(a &optional b)
                             `((or ,@gen-type
                                   ,@gen-itype ,@gen-utype
                                   ,@mat)
                               (=s 0))
                             '(= 0))
    #++
    (add-internal-function/s 'cl-webgpu/shader/internal::decf '(a &optional b)
                             `((or ,@gen-type
                                   ,@gen-itype ,@gen-utype
                                   ,@mat)
                               (=s 0))
                             '(= 0))
    (add-internal-function/s 'cl-webgpu/shader/internal::++ '(a)
                             `((or ,@gen-type
                                   ,@gen-itype ,@gen-utype
                                   ,@mat))
                             '(= 0)
    ...
    (add-internal-function/full 'and '(a b) '(((:bool :bool) :bool))
                                :cast nil)
    (add-internal-function/full 'cl-webgpu/shader/internal::^^ '(a b) '(((:bool :bool) :bool))
                                :cast nil)
    ...
    (add-internal-function/s 'cl-webgpu/shader/internal::|?:| '(c then else)
                             '(:bool T (= 1)) '(= 1))
    ...
    ;;    signed vs unsigned
    (add-internal-function/full 'cl-webgpu/shader/internal::<< '(integer count)
                                `(((:int :int) :int)
                                  ((:int :uint) :int)
                                  ((:uint :int) :uint)
                                  ((:uint :uint) :uint)
    ...
    (add-internal-function/full 'cl-webgpu/shader/internal::ash '(integer count)
                                `(((:int :int) :int)
                                  ((:int :uint) :int)
                                  ((:uint :int) :uint)
                                  ((:uint :uint) :uint)
    ...
    ;; fixme: most of these belong in cl-webgpu/shader/internal: package
    (macrolet ((add/s (&rest definitions)
                 `(progn
                    ,@(loop for (.name lambda-list arg-types return-type)
                              in definitions
                            for (name target-name . keys)
               ...
               (add/m (&rest definitions)
                 `(progn
                    ,@(loop for (.name lambda-list count ret)
                              in definitions
                            for (name target-name) = (alexandria:ensure-list
                                                    .name)
      ...
      (add/s
       ...
       (cl-webgpu/shader/internal:radians (degrees) `((or ,@gen-type)) `(= 0))
       (cl-webgpu/shader/internal:degrees (radians) `((or ,@gen-type)) `(= 0))
       ...
       (cl-webgpu/shader/internal::pow (x y) `((or ,@gen-type) (= 0)) `(= 0))
       (exp (x) `((or ,@gen-type)) `(= 0))
       (log (x) `((or ,@gen-type)) `(= 0))
       (cl-webgpu/shader/internal:exp2 (x) `((or ,@gen-type)) `(= 0))
       (cl-webgpu/shader/internal:log2 (x) `((or ,@gen-type)) `(= 0))
       (sqrt (x) `((or ,@gen-type)) `(= 0))
       ((cl-webgpu/shader/internal:inverse-sqrt "inversesqrt") (x)
        `((or ,@gen-type)) `(= 0))

       ;; 8.3 common functions
       (abs (x) `((or ,@gen-type ,@gen-itype)) `(= 0))
       ;; cl:signum -> cl-webgpu/shader/internal:sign
       (signum (x) `((or ,@gen-type ,@gen-itype)) `(= 0))
       (cl-webgpu/shader/internal:sign (x) `((or ,@gen-type ,@gen-itype)) `(= 0))
       ;; todo: compiler macro to expand (floor number divisor) to
       ;; (floor (/ number divisor))?
       ;; (and same for truncate, round, ceiling, etc)
       (floor (x) `((or ,@gen-type)) `(= 0))
       ;; cl:truncate -> cl-webgpu/shader/internal::trunc
       (truncate (x) `((or ,@gen-type)) `(= 0))
       (cl-webgpu/shader/internal:trunc (x) `((or ,@gen-type)) `(= 0))
       (round (x) `((or ,@gen-type)) `(= 0))
       (cl-webgpu/shader/internal:round-even (x) `((or ,@gen-type)) `(= 0))
       (ceiling (x) `((or ,@gen-type)) `(= 0))
       (cl-webgpu/shader/internal:ceil (x) `((or ,@gen-type)) `(= 0))
       (cl-webgpu/shader/internal:fract (x) `((or ,@gen-type)) `(= 0))
       ...
       (mod (x y) `((or ,@gen-itype ,@gen-utype
                        ,@gen-type) (=s 0)) `(= 0))
       (cl-webgpu/shader/internal:modf (x y) `((or ,@gen-type) (:out (=s 0)))
                        `(= 0))


       (min (x y) `((or ,@gen-type ,@gen-itype ,@gen-utype) (=s 0))
            `(= 0))
       (max (x y) `((or ,@gen-type ,@gen-itype ,@gen-utype) (=s 0))
            `(= 0))

       (cl-webgpu/shader/internal:clamp (x min max)
                         `((or ,@gen-type ,@gen-itype ,@gen-utype)
                           (=s 0) (=s 0)) '(= 0)))

      (add-internal-function/full 'cl-webgpu/shader/internal:mix '(x y a)
                                  (make-ftype
                                   ` (:float
                                      ,@vec ,@vec
                                      ,@gen-type
      ...
      (add/s
       (cl-webgpu/shader/internal:step (edge x) `((=s 1) (or ,@gen-type)) '(= 1))
       ((cl-webgpu/shader/internal:smooth-step "smoothstep") (edge0 edge1 x) `((=s 2)
                                                                (= 0)
                                                                (or ,@gen-type))
        '(= 2))
       ((cl-webgpu/shader/internal:is-nan "isnan") (x) `((or ,@gen-type))
        `(=# 0 :bool))
       ((cl-webgpu/shader/internal:is-inf "isinf") (x) `((or ,@gen-type))
        `(=# 0 :bool))
       (cl-webgpu/shader/internal:float-bits-to-int (value) `((or ,@gen-type))
                                     `(=# 0 :int))
       (cl-webgpu/shader/internal:float-bits-to-uint (value) `((or ,@gen-type))
                                      `(=# 0 :uint))
       (cl-webgpu/shader/internal:int-bits-to-float (value) `((or ,@gen-itype))
                                     `(=# 0 :float))
       (cl-webgpu/shader/internal:uint-bits-to-float (value) `((or ,@gen-utype))
                                      `(=# 0 :float))
       (cl-webgpu/shader/internal:fma (a b c) `((or ,@gen-type) (= 0) (= 0)) '(= 0))
       (cl-webgpu/shader/internal:frexp (x exp) `((or ,@gen-type) (:out (=# 0 :int)))
                         '(= 0))
       (cl-webgpu/shader/internal:ldexp (x exp) `((or ,@gen-type) (=# 0 :int))
                         '(= 0))

       ;; 8.4 floating-point pack and unpack functions
       (cl-webgpu/shader/internal:pack-unorm-2x16 (v) `(:vec2) :uint)
       (cl-webgpu/shader/internal:pack-snorm-2x16 (v) `(:vec2) :uint)
       (cl-webgpu/shader/internal:pack-unorm-4x8 (v) `(:vec2) :uint)
       (cl-webgpu/shader/internal:pack-snorm-4x8 (v) `(:vec2) :uint)

       (cl-webgpu/shader/internal:unpack-unorm-2x16 (v) `(:uint) :vec2)
       (cl-webgpu/shader/internal:unpack-snorm-2x16 (v) `(:uint) :vec2)
       (cl-webgpu/shader/internal:unpack-unorm-4x8 (v) `(:uint) :vec2)
       (cl-webgpu/shader/internal:unpack-snorm-4x8 (v) `(:uint) :vec2)

       (cl-webgpu/shader/internal:pack-half-2x16 (v) `(:vec2) :uint)
       (cl-webgpu/shader/internal:unpack-half-2x16 (v) `(:uint) :vec2)

       ;; 8.5 geometric functions
       ;; geometric length, not count of elements in sequence
       (cl-webgpu/shader/internal:length (x) `((or ,@gen-type)) '(s 0))
       (cl-webgpu/shader/internal:distance (p0 p1) `((or ,@gen-type) (= 0)) '(s 0))
       (cl-webgpu/shader/internal:dot (x y) `((or ,@gen-type) (= 0)) '(s 0))
       (cl-webgpu/shader/internal:cross (x y) `((or :vec3) (= 0)) '(= 0))
       (cl-webgpu/shader/internal:normalize (x) `((or ,@gen-type)) '(= 0))
       ((cl-webgpu/shader/internal:face-forward "faceforward") (n i n-ref)
        `((or ,@gen-type)
          (= 0)
          (= 0))
        '(= 0))
       (cl-webgpu/shader/internal:reflect (i n) `((or ,@gen-type) (= 0)) '(= 0))
       (cl-webgpu/shader/internal:refract (i n eta) `((or ,@gen-type) (= 0) :float)
                           '(= 0))


       ;; 8.6 matrix functions
       (cl-webgpu/shader/internal::matrix-comp-mult (x y) `((or ,@mat) (= 0)) '(= 0)))

      (add-internal-function/full 'cl-webgpu/shader/internal::outer-product '(x y)
                                  (make-ftype
                                   '(:mat2 :mat3 :mat4
                                     :mat2x3 :mat3x2
                                     :mat2x4 :mat4x2
      ...
      (add-internal-function/full 'cl-webgpu/shader/internal::transpose '(m)
                                  (make-ftype
                                   '(:mat2 :mat3 :mat4
                                     :mat2x3 :mat3x2
                                     :mat2x4 :mat4x2
      ...
      (add/s
       (cl-webgpu/shader/internal::determinant (m) `((or :mat2 :mat3 :mat4))
                               '(= 0))
       (cl-webgpu/shader/internal::inverse (m) `((or :mat2 :mat3 :mat4)) '(= 0))


       ;; 8.7 vector relational functions
       (cl-webgpu/shader/internal::less-than (x y) `((or ,@vec ,@ivec ,@uvec) (= 0))
                             '(=# 0 :bool))
       (cl-webgpu/shader/internal::less-than-equal (x y) `((or ,@vec ,@ivec ,@uvec)
                                           (= 0))
                                   '(=# 0 :bool))
       (cl-webgpu/shader/internal::greater-than (x y) `((or ,@vec ,@ivec ,@uvec) (= 0))
                                '(=# 0 :bool))
       (cl-webgpu/shader/internal::greater-than-equal (x y) `((or ,@vec ,@ivec ,@uvec)
                                              (= 0))
                                      '(=# 0 :bool))
       ;; component-wise compare, unlike cl:equal
       (cl-webgpu/shader/internal::equal (x y) `((or ,@vec ,@ivec ,@uvec ,@bvec) (= 0))
                         '(=# 0 :bool))
       (cl-webgpu/shader/internal::not-equal (x y) `((or ,@vec ,@ivec ,@uvec ,@bvec)
                                     (= 0))
                             '(=# 0 :bool))
       (cl-webgpu/shader/internal::any (x) `((or ,@bvec)) :bool)
       (cl-webgpu/shader/internal::all (x) `((or ,@bvec)) :bool)
       ;; component-wise negation, unlike cl:not
       ;; merged with cl:not for now?
       ;; (cl-webgpu/shader/internal::not (x) `((or ,@bvec)) '(= 0))


       ;; 8.8 integer functions
       (cl-webgpu/shader/internal::uadd-carry (x y carry) `((or ,@gen-utype) (= 0) (:out (= 0)))
                              '(= 0))
       (cl-webgpu/shader/internal::usub-borrow (x y borrow) `((or ,@gen-utype) (= 0)
                                              (:out (= 0)))
                               '(= 0))
       (cl-webgpu/shader/internal::umul-extended (x y msb lsb) `((or ,@gen-utype) (= 0)
                                                 (:out (= 0)) (:out (= 0)))
                                 '(= 0))
       (cl-webgpu/shader/internal::imul-extended (x y msb lsb) `((or ,@gen-itype) (= 0)
                                                 (:out (= 0)) (:out (= 0)))
                                 '(= 0))
       ;; todo: expand LDB/DPB to these?
       (cl-webgpu/shader/internal::bitfield-extract (value offset bits)
                                    `((or ,@gen-itype ,@gen-utype) :int :int)
                                    '(= 0))
       (cl-webgpu/shader/internal::bitfield-insert (base insert offset bits)
                                   `((or ,@gen-itype ,@gen-utype) (= 0)
                                     :int :int)
                                   '(= 0))
       (cl-webgpu/shader/internal::bitfield-reverse (value) `((or ,@gen-itype ,@gen-utype))
                                    '(= 0))
       (cl-webgpu/shader/internal::bit-count (value) `((or ,@gen-itype ,@gen-utype))
                             '(=# 0 :int))
       ((cl-webgpu/shader/internal::find-lsb "findLSB") (value) `((or ,@gen-itype ,@gen-utype))
        '(=# 0 :int))
       ((cl-webgpu/shader/internal::find-msb "findMSB") (value) `((or ,@gen-itype ,@gen-utype))
        '(=# 0 :int)))

      ;; 8.9 Texture Functions
      (add/f
       (cl-webgpu/shader/internal::texture-size (sampler &optional lod)
       ...
       (cl-webgpu/shader/internal::texture-query-lod (sampler p)
                                     (expand-signatures
       ...
       (cl-webgpu/shader/internal::texture-query-levels '(sampler)
                                        (expand-signatures
                                         :int (gsampler2D)
                                         :int (gsampler3D)
                                         :int (gsamplerCube)
      ...
      ;; WGSL-native textureSample with separate texture + sampler args
      (add/f
       ((cl-webgpu/shader/internal::texture-sample "textureSample")
        (tex samp coord &optional extra)
        (expand-signatures/o (3)
         :vec4 (:sampler-2d :sampler :vec2 :float)
         :vec4 (:sampler-3d :sampler :vec3 :float)
         :vec4 (:sampler-cube :sampler :vec3 :float)
         ;; WGSL: textureSample(t, s, coords: vec2, array_index: i32/u32)
         :vec4 (:sampler-2d-array :sampler :vec2 :int)
         :vec4 (:sampler-2d-array :sampler :vec2 :uint)
         :float (:sampler-2d-shadow :sampler :vec2 :float))))

      (add/f
       (cl-webgpu/shader/internal::texture (sampler p &optional bias/compare)
       ...
       (cl-webgpu/shader/internal::texture-proj (sampler p &optional bias)
                                (expand-signatures/o (2)
       ...
       (cl-webgpu/shader/internal::texture-lod (sampler p lod)
                               (expand-signatures
                                gvec4 (gsampler2D :vec2 :float)
                                gvec4 (gsampler3D :vec3 :float)
                                gvec4 (gsamplerCube :vec3 :float)
                                ...
                                gvec4 (gsampler2DArray :vec3 :float)
                                gvec4 (gsamplerCubeArray :vec4 :float)))
       (cl-webgpu/shader/internal::texture-offset (sampler p offset &optional bias)
                                  (expand-signatures/o (3)
       ...
       (cl-webgpu/shader/internal::texel-fetch (sampler p &optional lod/sample)
                               (expand-signatures/o (2)
                                gvec4 (gsampler2D :ivec2 :int)
                                gvec4 (gsampler3D :ivec3 :int)
                                gvec4 (gsampler2DArray :ivec3 :int)))

       (cl-webgpu/shader/internal::texel-fetch-offset (sampler P lod offset)
                                      (expand-signatures
                                       gvec4 (gsampler2D :ivec2 :int :ivec2)
                                       gvec4 (gsampler3D :ivec3 :int :ivec3)
                                       gvec4 (gsampler2DArray :ivec3 :int :ivec2)))

       (cl-webgpu/shader/internal::texture-proj-offset (sampler p offset &optional bias)
                                       (expand-signatures/o (3)
                                        ...
                                        :float (:sampler-2D-Shadow
                                                :vec4 :ivec2 :float)))
       (cl-webgpu/shader/internal::texture-lod-offset (sampler p lod offset)
                                      (expand-signatures
                                       ...
                                       gvec4 (gsampler2DArray
                                              :vec3 :float :ivec2)))
       (cl-webgpu/shader/internal::texture-proj-lod (sampler o lod)
                                    (expand-signatures
                                     gvec4 (gsampler2D :vec3 :float)
                                     gvec4 (gsampler2D :vec4 :float)
                                     gvec4 (gsampler3D :vec4 :float)
                                     :float (:sampler-2D-Shadow
                                             :vec4 :float)))
       (cl-webgpu/shader/internal::texture-proj-lod-offset
       ...
       (cl-webgpu/shader/internal::texture-grad (sampler p dP/dx dP/dy)
                                (expand-signatures
       ...
       (cl-webgpu/shader/internal::texture-grad-offset
       ...
       (cl-webgpu/shader/internal::texture-proj-grad
       ...
       (cl-webgpu/shader/internal::texture-proj-grad-offset
        (sampler p dp/dx dp/dy offset)
        (expand-signatures
       ...
       (cl-webgpu/shader/internal::texture-gather (sampler p refz)
                                  (expand-signatures
                                   ...
                                   :vec4 (:sampler-Cube-Array-Shadow
                                          :vec4 :float)))
       (cl-webgpu/shader/internal::texture-gather-offset
       ...
       #++
       (cl-webgpu/shader/internal::texture-gather-offsets
      ...
      (add/s
       ((cl-webgpu/shader/internal::texture-2d "texture2D")
        (sampler coord &optional bias) '(:sampler-2d :vec2 :float) :vec4)
       ((cl-webgpu/shader/internal::texture-2d-proj "texture2DProj")
        (sampler coord &optional bias)
        '(:sampler-2d (or :vec3 :vec4) :float) :vec4)
       ((cl-webgpu/shader/internal::texture-2d-lod "texture2DLod") (sampler coord lod)
        '(:sampler-2d :vec2 :float) :vec4)
       ((cl-webgpu/shader/internal::texture-2d-proj-lod "texture2DProjLod") (sampler coord lod)
        '(:sampler-2d (or :vec3 :vec4) :float)
        :vec4)

       ((cl-webgpu/shader/internal::texture-3d "texture3D") (sampler coord &optional bias)
        '(:sampler-3d :vec3 :float) :vec4)
       ((cl-webgpu/shader/internal::texture-3d-proj "texture3DProj")
        (sampler coord &optional bias)
        '(:sampler-3d :vec4 :float) :vec4)
       ((cl-webgpu/shader/internal::texture-3d-lod "texture3DLod") (sampler coord lod)
        '(:sampler-3d :vec3 :float) :vec4)
       ((cl-webgpu/shader/internal::texture-3d-proj-lod "texture3DProjLod") (sampler coord lod)
        '(:sampler-3d :vec4 :float) :vec4)

       (cl-webgpu/shader/internal::texture-cube (sampler coord &optional bias)
                                '(:sampler-3d :vec3 :float) :vec4)
       (cl-webgpu/shader/internal::texture-cube-lod (sampler coord lod)
                                    '(:sampler-3d :vec3 :float) :vec4))
      ...
      (add/s
       ...
       (cl-webgpu/shader/internal::atomic-add (mem data) '((or :uint :int) (= 0)) '(= 0))
       (cl-webgpu/shader/internal::atomic-min (mem data) '((or :uint :int) (= 0)) '(= 0))
       (cl-webgpu/shader/internal::atomic-max (mem data) '((or :uint :int) (= 0)) '(= 0))
       (cl-webgpu/shader/internal::atomic-and (mem data) '((or :uint :int) (= 0)) '(= 0))
       (cl-webgpu/shader/internal::atomic-or (mem data) '((or :uint :int) (= 0)) '(= 0))
       (cl-webgpu/shader/internal::atomic-xor (mem data) '((or :uint :int) (= 0)) '(= 0))
       (cl-webgpu/shader/internal::atomic-exchange (mem data) '((or :uint :int) (= 0)) '(= 0))
       (cl-webgpu/shader/internal::atomic-comp-swap (mem compare data)
                                    '((or :uint :int) (= 0) (= 0)) '(= 0)))
      ...
      (add/s
       ...
       (cl-webgpu/shader/internal::interpolate-at-centroid (interpolant) `((or ,@gen-type)) '(= 0))
       (cl-webgpu/shader/internal::interpolate-at-sample (interpolant sample)
                                         `((or ,@gen-type) :int) '(= 0))
       (cl-webgpu/shader/internal::interpolate-at-centroid (interpolant offset)
                                           `((or ,@gen-type) :vec2) '(= 0)))
      ...
      (add/s
       (cl-webgpu/shader/internal::barrier () () :void))

      ;; 8.17 Shader memory control functions
      ;; all shader types
      (add/s
       (cl-webgpu/shader/internal::memory-barrier () () :void)
       (cl-webgpu/shader/internal::memory-barrier-buffer () () :void)
       (cl-webgpu/shader/internal::memory-barrier-shared () () :void)
       (cl-webgpu/shader/internal::memory-barrier-image () () :void)
       (cl-webgpu/shader/internal::group-memory-barrier () () :void))

      ;; 8.19 Shader Invocation Group Functions
      (add/s
       (cl-webgpu/shader/internal::any-invocation (value) `(:bool) :bool)
       (cl-webgpu/shader/internal::all-invocations (value) `(:bool) :bool)
       (cl-webgpu/shader/internal::all-invocations-equal (value) `(:bool) :bool))
      ...
      (add/s
       ...
       (cl-webgpu/shader/internal::int (x) `((or :int :uint :bool :float
                      ,@ivec ,@uvec ,@vec ,@mat)) :int)
       (cl-webgpu/shader/internal::uint (x) `((or :int :uint :bool :float
                       ,@ivec ,@uvec ,@vec ,@mat)) :uint)
       (cl-webgpu/shader/internal::bool (x) `((or :int :uint :bool :float
                       ,@ivec ,@uvec ,@vec ,@mat)) :bool)
       (float (x) `((or :int :uint :bool :float
                        ,@ivec ,@uvec ,@vec ,@mat)) :float))
      ...
      (add/m
       (cl-webgpu/shader/internal::bvec2 (a &optional b) 2 :bvec2)
       (cl-webgpu/shader/internal::bvec3 (a &optional b c) 3 :bvec3)
       (cl-webgpu/shader/internal::bvec4 (a &optional b c d) 4 :bvec4)

       (cl-webgpu/shader/internal::i8vec2 (a &optional b) 2 :i8vec2)
       (cl-webgpu/shader/internal::i8vec3 (a &optional b c) 3 :i8vec3)
       (cl-webgpu/shader/internal::i8vec4 (a &optional b c d) 4 :i8vec4)
       (cl-webgpu/shader/internal::u8vec2 (a &optional b) 2 :u8vec2)
       (cl-webgpu/shader/internal::u8vec3 (a &optional b c) 3 :u8vec3)
       (cl-webgpu/shader/internal::u8vec4 (a &optional b c d) 4 :u8vec4)

       (cl-webgpu/shader/internal::i16vec2 (a &optional b) 2 :i16vec2)
       (cl-webgpu/shader/internal::i16vec3 (a &optional b c) 3 :i16vec3)
       (cl-webgpu/shader/internal::i16vec4 (a &optional b c d) 4 :i16vec4)
       (cl-webgpu/shader/internal::u16vec2 (a &optional b) 2 :u16vec2)
       (cl-webgpu/shader/internal::u16vec3 (a &optional b c) 3 :u16vec3)
       (cl-webgpu/shader/internal::u16vec4 (a &optional b c d) 4 :u16vec4)

       (cl-webgpu/shader/internal::ivec2 (a &optional b) 2 :ivec2)
       (cl-webgpu/shader/internal::ivec3 (a &optional b c) 3 :ivec3)
       (cl-webgpu/shader/internal::ivec4 (a &optional b c d) 4 :ivec4)
       (cl-webgpu/shader/internal::uvec2 (a &optional b) 2 :uvec2)
       (cl-webgpu/shader/internal::uvec3 (a &optional b c) 3 :uvec3)
       (cl-webgpu/shader/internal::uvec4 (a &optional b c d) 4 :uvec4)

       (cl-webgpu/shader/internal::i64vec2 (a &optional b) 2 :i64vec2)
       (cl-webgpu/shader/internal::i64vec3 (a &optional b c) 3 :i64vec3)
       (cl-webgpu/shader/internal::i64vec4 (a &optional b c d) 4 :i64vec4)
       (cl-webgpu/shader/internal::u64vec2 (a &optional b) 2 :u64vec2)
       (cl-webgpu/shader/internal::u64vec3 (a &optional b c) 3 :u64vec3)
       (cl-webgpu/shader/internal::u64vec4 (a &optional b c d) 4 :u64vec4)

       (cl-webgpu/shader/internal::f16vec2 (a &optional b) 2 :f16vec2)
       (cl-webgpu/shader/internal::f16vec3 (a &optional b c) 3 :f16vec3)
       (cl-webgpu/shader/internal::f16vec4 (a &optional b c d) 4 :f16vec4)

       (cl-webgpu/shader/internal::vec2 (a &optional b) 2 :vec2)
       (cl-webgpu/shader/internal::vec3 (a &optional b c) 3 :vec3)
       (cl-webgpu/shader/internal::vec4 (a &optional b c d) 4 :vec4)

       (cl-webgpu/shader/internal::mat2 (a &optional b c d) 4 :mat2)
       (cl-webgpu/shader/internal::mat2x3 (a &optional b c d e f) 6 :mat2x3)
       (cl-webgpu/shader/internal::mat2x4 (a &optional b c d e f g h) 8 :mat2x4)
       (cl-webgpu/shader/internal::mat3x2 (a &optional b c d e f) 6 :mat3x2)
       (cl-webgpu/shader/internal::mat3 (a &optional b c d e f g h i) 9  :mat3)
       (cl-webgpu/shader/internal::mat3x4 (a &optional b c d e f g h i j k l) 12  :mat3x4)
       (cl-webgpu/shader/internal::mat4x2 (a &optional b c d e f g h) 8  :mat4x2)
       (cl-webgpu/shader/internal::mat4x3 (a &optional b c d e f g h i j k l) 12 :mat4x3)
       (cl-webgpu/shader/internal::mat4 (a &optional b c d e f g h i j k l m n o p) 16 :mat4))
      ...
      (add/s
       ...
       ;; void functions
       (cl-webgpu/shader/internal::discard () '() t)))))
...
(%glsl-compiler-macro 'cl-webgpu/shader/internal::incf (a &optional b)
  `(setf ,a (+ ,a ,(or b 1))))
(%glsl-compiler-macro 'cl-webgpu/shader/internal::decf (a &optional b)
  `(setf ,a (- ,a ,(or b 1))))
