...
(defclass infer-build-constraints (cl-webgpu/shader/internal::shader-walker)
  ())
...
(defmethod walk ((form if-form) (walker infer-build-constraints))
  ...
  (if (and (then-form form) (else-form form))
      ;; Walk both branches but don't unify their types.  In WGSL if
      ;; is always a statement, so branches can have different types.
      ;; When the if result IS used as an expression (e.g. in a let
      ;; binding), the parent context provides type constraints.
      (let ((then-type (walk (then-form form) walker)))
  ...
  (let ((struct-type (walk (binding form) walker)))
    ...
    (loop for binding in (bindings struct-type)
          when (or (eql (name binding) (field form))
                   ...
                   (string= (string (name binding)) (string (field form))))
    ...
    (cond
      ((eq (called-function form)
           (get-function-binding 'return
                                 :env cl-webgpu/shader/internal::*shader-base-environment*))
...
(defmethod walk ((form (eql t)) (walker infer-build-constraints))
  (set-type :bool))

(defmethod walk ((form null) (walker infer-build-constraints))
  (set-type :bool))
...
(defmethod walk ((form struct-construction) (walker infer-build-constraints))
  ;; struct construction returns its struct type
  (value-type form))
                  ...
                  (typecase type
                    (constrained-type
                     (let ((old (hash-table-count (types type)))
                           (orig-types (let (r) (maphash (lambda (k v) (when v (push (debug-type-names k) r))) (types type)) r)))
                       (maphash (lambda (k v)
                                  (unless (and v (gethash k cast-types))
                                    (remhash k (types type))))
                                (types type))
                       (when (zerop (hash-table-count (types type)))
                        (error "cast-constraint emptied type:~% original types: ~s~% cast-types: ~s~% in-type: ~s~% out-type: ~s~% cast-type: ~s"
                               orig-types
                               (let (r) (maphash (lambda (k v) (when v (push (debug-type-names k) r))) cast-types) r)
...
(multiple-value-list
 (compile-block '((defun h (a b)
                    (declare (:float b))
                    (cl-webgpu/shader/internal::vec3 a b)))
                   'h
                   :vertex))
...
(multiple-value-list
 (compile-block '((defun h (a b)
                    (declare (:float b))
                    (+ (cl-webgpu/shader/internal::vec3 a b) 2.0)))
                   'h
                   :vertex))
...
(multiple-value-list
 (compile-block '((defun h (a b)
                    (declare (:float b))
                    (+ (cl-webgpu/shader/internal::mat3 a b) 2)))
                'h
                :vertex))
...
(multiple-value-list
 (compile-block '((defun h (a b c)
                    ...
                    (cl-webgpu/shader/internal::smooth-step a b c)))
                   'h
                   :vertex))
...
(multiple-value-list
 (compile-block '((defun h ()
                    (let ((a))
                      (setf a (1+ (cl-webgpu/shader/internal::vec4 1 2 3 4))))))
                'h
                :vertex))
...
(multiple-value-list
 (compile-block '((defun h ()
                    (let ((a))
                      (setf (.rb a) (1+ (cl-webgpu/shader/internal::vec2 1 2))))))
                'h
                :vertex))
  ...
  (compile-block '((defun h ()
                     ...
                     (let ((a))
                       (setf (.b a) 1
                             (.r a) 2)
                       (if (>= (.b a) 1)
                           (return (cl-webgpu/shader/internal:vec4 1 2 3 4))
                           (return a)))))
                 'h
                 :vertex)))
...
(print
 (multiple-value-list
  (compile-block '((defun h ()
                     (return (cl-webgpu/shader/internal:vec2 1))
                     (return (cl-webgpu/shader/internal:ivec2 1))))
                 'h
                 :vertex)))
...
(print
 (multiple-value-list
  (compile-block '((defun p1 (g)
                     (if g
                         (return (cl-webgpu/shader/internal::ivec2 1 2))
                         (return (cl-webgpu/shader/internal::vec2 1 2))))
                   (defun h ()
                     (let ((a (p1 (> 1 2))))
                       ;(return a)
                       )))
...
#++
(gethash *package* cl-webgpu/shader/internal::*package-environments*)
#++
(remhash *package* cl-webgpu/shader/internal::*package-environments*)

#++
(progn
  (cl-webgpu/shader/internal:defconstant +foo+ 1 :float)

  (cl-webgpu/shader/internal:defun a (a1)
    +foo+
    (return a1))

  (cl-webgpu/shader/internal:defun b ()
    (a 1))
