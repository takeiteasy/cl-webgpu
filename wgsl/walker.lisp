...
(defvar cl-webgpu/shader/internal::*shader-base-environment*
  (make-instance 'environment
                 :parent *cl-environment*
                 :name 'base))
(setf (gethash (find-package :cl-webgpu/shader/internal) *package-environments*)
      cl-webgpu/shader/internal::*shader-base-environment*)

;; Unlock during initialization — re-locked at end of api.lisp
(setf (locked *cl-environment*) nil
      (locked cl-webgpu/shader/internal::*shader-base-environment*) nil)
  ...
  (or (gethash package *package-environments*)
      (setf (gethash package *package-environments*)
            (make-instance 'environment
                           ...
                           ;; configurable?
                           :parent cl-webgpu/shader/internal::*shader-base-environment*
                           :name package))))
  ...
  (and env
       (or (gethash name (variable-bindings env))
           ...
           (let ((symbol-env (gethash (symbol-package name)
                                      cl-webgpu/shader/internal::*package-environments*))
  ...
  (if (consp name)
      ...
      (and env
           (or (gethash name (types env))
               ;; Fall back to string-based lookup for cross-package struct refs
               (and (symbolp name)
                    (not (keywordp name))
                    (block nil
                      (maphash (lambda (k v)
...
(defmacro %glsl-compiler-macro (name lambda-list &body body)
  (let ((sym (if (and (consp name) (eq (car name) 'quote))
                 (cadr name)
                 name)))
    ;; Compiler macros are called as (funcall fn whole-form env).
    ;; Support &whole in lambda-list by converting to a proper 2-arg lambda.
    (if (eq '&whole (car lambda-list))
        (let ((whole-var (cadr lambda-list))
              (rest-ll (cddr lambda-list)))
          `(add-compiler-macro
            ',sym
            '(lambda (,whole-var %env%)
               (declare (ignorable %env%))
               (destructuring-bind ,rest-ll (cdr ,whole-var)
                 ,@body))))
        `(add-compiler-macro
          ',sym
          '(lambda (%form% %env%)
...
(defun make-&key-expander (lambda-list)
  (multiple-value-bind (req opt rest key aux)
    ...
    ;; else if we have only optional args, we let WGSL default values handle it
