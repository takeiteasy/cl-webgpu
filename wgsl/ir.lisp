...
;;     as WGSL `return` which exits entire function
;; throw/catch -- ignore
...
(defclass annotated-binding (binding)
  ((location :accessor binding-location :initform nil :initarg :location)
   (builtin :accessor binding-builtin :initform nil :initarg :builtin)))

(defclass struct-construction ()
  ((construction-type :accessor construction-type :initarg :struct-type)
   (value-type :accessor value-type :initarg :value-type)))
...
(defclass function-binding ()
  ...
  ((name :accessor name :initarg :name)
   ...
   (declared-type :accessor declared-type :initarg :declared-type
                  :initform t)
   ;; location annotation for return value (e.g. from (values :vec4 :location 0))
   (return-location :accessor return-location :initform nil)))
...
(defclass builtin-function (function-binding-function bindings)
  ;; declarations for functions provided by the shader language
  ;; (or external shader code)
  ())
