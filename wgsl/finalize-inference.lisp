...
(defclass finalize (cl-webgpu/shader/internal::shader-walker)
  ())
...
(defun finalize-inference (root)
  (let ((*instantiated-overloads* (make-hash-table))
    ...
    ;; For the root entry-point function, derive argument types from
    ;; its bindings' inferred types (e.g. struct input parameter).
    (let ((root-arg-types
            (mapcar (lambda (b)
                      (get-concrete-type
                       (gethash b (local-binding-type-data root))))
                    (bindings root))))
      (flatten-function root root-arg-types))
